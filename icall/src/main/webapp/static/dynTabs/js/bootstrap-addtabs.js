...
$.fn.addtabs = function (options) {
    ...
    obj.on('click','li',function() {
    	if($(this).parent().is('.dropdown-menu'))
    	{
    		...
    		$(this).insertAfter('#tab-home');
    		$('.close-tab',$(this)).css('display','block');
    		Addtabs.drop();
    	}
    });

    obj.on('mouseover','li',function() {
        $(this).find('.close-tab').css('color','#ff0000');
    });

    obj.on('mouseout','li',function() {
    	$(this).find('.close-tab').css('color','#ffcc00');
    });
    ...
    $(window).resize(function () {
        ...
        .not('.tabdrop')
        .each(function () {
        	console.log($('.close-tab',$(this)).css('display'));
        	if($('.close-tab',$(this)).css('display')=='none')
        	$('.close-tab',$(this)).css('display','block');
        });
        Addtabs.drop();
    });
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (!$("#" + id)[0]) {
            ...
            //加入TABS
            title.insertAfter('#tab-home');
            content.insertAfter('#content-home');
//            obj.children('.nav-tabs').prepend(title);
//            obj.children(".tab-content").append(content);
        ...
        $("#" + id).addClass("active");
        if($("#tab_" + id).parent().is('.dropdown-menu'))
    	{
        	$("#tab_" + id).insertAfter('#tab-home');
        	$("#" + id).insertAfter('#content-home');
    		$('.close-tab',$("#tab_" + id)).css('display','block');
    	}
        Addtabs.drop();
    },
    close: function (id) {
        ...
        element.append(dropdown.find('li'))
            ...
            .each(function () {
                if (this.offsetTop > 0 || element.width() - $(this).position().left - $(this).width() < 53) {
                    ...
                    $('.close-tab',$(this)).css('display','none');
                    collection++;
                }
            });
